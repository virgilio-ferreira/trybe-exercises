...
const reduceNames = (acc, book, index, array) => {
  if (index === array.length - 1) return `${acc} ${book.author.name}.`;// para colocar ponto no final da frase
  return `${acc} ${book.author.name},`;// tem uma virgula
};

function allNames() {
  console.log(books.reduce(reduceNames,'Nomes:'));
